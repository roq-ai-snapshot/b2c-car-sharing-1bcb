...
import { useRoqPlatformUserProfiles, useCarCount, useDashboardCount, useOperationCount } from 'lib/roq';
...
function DashboardPage() {
  ...
  const dashboardStat = useDashboardCount();

  const operationStat = useOperationCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Dashboards',
      result: dashboardStat?.data,
      isLoading: dashboardStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Operations',
      result: operationStat?.data,
