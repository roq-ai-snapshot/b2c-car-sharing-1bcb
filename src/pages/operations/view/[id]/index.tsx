...
import { useOperationFindFirst, useRoqClient } from 'lib/roq';
...
function OperationViewPage() {
  ...
  const queryParams = useMemo<{ include: { dashboard: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useOperationFindFirst(queryParams, {}, { disabled: !id });
              ...
              >
                ...
                {hasAccess('dashboard', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Dashboard"
                    text={
                      <Link as={NextLink} href={`/dashboards/view/${data?.dashboard?.id}`}>
                        {data?.dashboard?.name + ''}
                      </Link>
                    }
                  />
