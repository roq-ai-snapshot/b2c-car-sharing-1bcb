...
import { useRoqClient, useOperationFindFirst } from 'lib/roq';
...
function OperationEditPage() {
  ...
  const { data, error, isLoading, mutate } = useOperationFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: OperationInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.operation.update({
        data: values as RoqTypes.operation,
        where: {
          id,
        },
      });
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <AsyncSelect<DashboardInterface>
            formik={formik}
            name={'dashboard_id'}
            label={'Select Dashboard'}
            placeholder={'Select Dashboard'}
            fetcher={() => roqClient.dashboard.findManyWithCount({})}
            labelField={'name'}
          />
