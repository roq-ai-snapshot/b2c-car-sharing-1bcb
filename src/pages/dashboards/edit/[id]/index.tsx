...
import { useRoqClient, useDashboardFindFirst } from 'lib/roq';
...
function DashboardEditPage() {
  ...
  const { data, error, isLoading, mutate } = useDashboardFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DashboardInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.dashboard.update({
        data: values as RoqTypes.dashboard,
        where: {
          id,
        },
      });
